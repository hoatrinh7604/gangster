...
public class OneSignalHandler : MonoBehaviour {
    ...
    void Start()
    {
        //  291b3bc9-6d71-499c-825c-fab81d35e67e
        ...
        OneSignal.StartInit(AdsManager.Instance.onesignalAppId)
    ...
    public static void HandleNotificationOpened(OSNotificationOpenedResult result)
    {
        ...
        else
        {
            ...
            if(additionalData.ContainsKey("package"))
            {
                string packageName = (string)additionalData["package"];
                Debug.Log("Package Name :: " + packageName);
            }

            if (additionalData.ContainsKey("incentive"))
            {
                int coins = int.Parse((string)additionalData["incentive"]);
                Debug.Log("Incentive bonus :: " + coins);
            }

...
public class LanguageHandler : MonoBehaviour {
	...
	public static int currentLanguageIndex {
        get
        {
            return PlayerPrefs.GetInt(GameConstants.LANGUAGE_INDEX, 0);
        }
        set
        {
            PlayerPrefs.SetInt(GameConstants.LANGUAGE_INDEX, value);
        }
    }
	...
	public void SetLanguageIndex() {
        if (!PlayerPrefs.HasKey(GameConstants.LANGUAGE_INDEX))
        {
	...
	public void SetLanguageIndex(string language) {
		...
		for (int i = 0; i < TextReader.numberOfLanguages; i++) {
			if (language.Equals (TextReader.allLanguages [i].name)) {
	...
	{
		return TextReader.allLanguages [currentLanguageIndex].ValueOfKey (key);
	}
